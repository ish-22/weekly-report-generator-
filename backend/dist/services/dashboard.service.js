...
class DashboardService {
    static async getDashboardMetrics() {
        ...
        const [totalTeamMembers, submittedThisWeek, pendingReviewCount, needsCorrectionCount, approvedCount, openBlockersCount, totalHoursThisWeek,] = await Promise.all([
            // Total active team members
            prisma_lib_1.prisma.user.count({
                where: { role: { name: client_1.RoleName.TEAM_MEMBER }, isActive: true },
            }),
            // Total submitted/approved reports for current week
            prisma_lib_1.prisma.report.count({
                where: {
                    weekStartDate: { gte: currentWeekStart },
                    status: { in: [client_1.ReportStatus.SUBMITTED, client_1.ReportStatus.APPROVED] },
                },
            }),
            // Reports currently pending review (SUBMITTED status)
            prisma_lib_1.prisma.report.count({
                where: { status: client_1.ReportStatus.SUBMITTED },
            }),
            // Reports currently in NEEDS_CORRECTION status
            prisma_lib_1.prisma.report.count({
                where: { status: client_1.ReportStatus.NEEDS_CORRECTION },
            }),
            ...
            // Open blockers count
            prisma_lib_1.prisma.report.count({
                where: { isKeyBlocker: true, status: { not: client_1.ReportStatus.APPROVED } },
            }),
            // Total time spent hours logged in current week
            prisma_lib_1.prisma.reportTask.aggregate({
                where: {
                    report: {
                        weekStartDate: { gte: currentWeekStart },
                    },
    ...
    static async getAnalyticsCharts() {
        ...
        const taskTypeAggregations = await prisma_lib_1.prisma.reportTask.groupBy({
            by: ['taskType'],
        ...
        const reportStatusGroup = await prisma_lib_1.prisma.report.groupBy({
            by: ['status'],
            _count: { id: true },
        });
        ...
        const projectWorkload = await prisma_lib_1.prisma.project.findMany({
            where: { isActive: true },
            select: {
                id: true,
                name: true,
        ...
        const taskStatusCounts = await prisma_lib_1.prisma.reportTask.groupBy({
            by: ['status'],
            _count: { id: true },
        });
        ...
        const recentReviews = await prisma_lib_1.prisma.reviewHistory.findMany({
            take: 10,
